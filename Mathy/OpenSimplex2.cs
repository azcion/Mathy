...
public static class OpenSimplex2 {
	...
	private static float Noise2_UnskewedBase (long seed, double xs, double ys) {
		...
		if (a0 > 0) value = a0 * a0 * (a0 * a0) * Grad(seed, xsbp, ysbp, dx0, dy0);
		...
		if (dy0 > dx0) {
			...
			if (a2 > 0) value += a2 * a2 * (a2 * a2) * Grad(seed, xsbp, ysbp + PrimeY, dx2, dy2);
		} else {
			float dx2 = dx0 - (float)(Unskew2D + 1);
			float dy2 = dy0 - (float)Unskew2D;
			float a2 = Rsquared2D - dx2 * dx2 - dy2 * dy2;
			if (a2 > 0) value += a2 * a2 * (a2 * a2) * Grad(seed, xsbp + PrimeX, ysbp, dx2, dy2);
		}
	...
	private static float Noise3_UnrotatedBase (long seed, double xr, double yr, double zr) {
		...
		for (int l = 0;; l++) {
			...
			if (a > 0) value += a * a * (a * a) * Grad(seed, xrbp, yrbp, zrbp, xri, yri, zri);
    ...
    */
	...
	static OpenSimplex2 () {
		...
		for (int i = 0; i < grad2.Length; i++) grad2[i] = (float)(grad2[i] / Normalizer2D);
		...
		for (int i = 0; i < grad3.Length; i++) grad3[i] = (float)(grad3[i] / Normalizer3D);
		...
		for (int i = 0; i < grad4.Length; i++) grad4[i] = (float)(grad4[i] / Normalizer4D);
